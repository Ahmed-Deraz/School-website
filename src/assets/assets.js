import about from "./about.jpg";
import american_head from "./american-head.jpg";
import back_icon from "./back-icon.png";
import close_sign from "./close-sign.png";
import dark_arrow from "./dark-arrow.png";
import elementary_american from "./elementary-american.jpg";
import facebook_icon from "./facebook-icon.png";
import gallery_1 from "./gallery-1.jpg";
import gallery_2 from "./gallery-2.jpg";
import gallery_3 from "./gallery-3.jpg";
import gallery_4 from "./gallery-4.jpg";
import graduates from "./graduates.png";
import hero_american from "./hero-american.jpg";
import hero from "./hero.png";
import highschool_american from "./highschool-american.jpg";
import instagram_icon from "./instagram-icon.png";
import keystage_american from "./keystage-american.jpg";
import linkedin_icon from "./linkedin-icon.png";
import location_icon from "./location-icon.png";
import logo from "./logo.png";
import mail_icon from "./mail-icon.png";
import menu_icon from "./menu-icon.png";
import middle_american from "./middle-american.jpg";
import msg_icon from "./msg-icon.png";
import next_icon from "./next-icon.png";
import phone_icon from "./phone-icon.png";
import play_icon from "./play-icon.png";
import playschool_american from "./playschool-american.jpg";
import program_1 from "./program-1.jpg";
import program_2 from "./program-2.jpg";
import program_3 from "./program-3.jpg";
import students from "./students.png";
import teachers from "./teachers.png";
import user_1 from "./user-1.png";
import user_2 from "./user-2.png";
import user_3 from "./user-3.png";
import user_4 from "./user-4.png";
import white_arrow from "./white-arrow.png";
import years from "./years.png";
import hero_british from "./hero-british.jpg";
import school_head from "./school-head.jpg";
import mission from "./mission.png";
import vision from "./vision.png";
import core_value from "./core-value.png";

export const assets = {
  about,
  american_head,
  back_icon,
  close_sign,
  dark_arrow,
  elementary_american,
  facebook_icon,
  gallery_1,
  gallery_2,
  gallery_3,
  gallery_4,
  graduates,
  hero_american,
  hero,
  highschool_american,
  instagram_icon,
  keystage_american,
  linkedin_icon,
  location_icon,
  logo,
  mail_icon,
  menu_icon,
  middle_american,
  msg_icon,
  next_icon,
  phone_icon,
  play_icon,
  playschool_american,
  program_1,
  program_2,
  program_3,
  students,
  teachers,
  user_1,
  user_2,
  user_3,
  user_4,
  white_arrow,
  years,
  mission,
  vision,
  core_value,
};

export const departmentsAmerican = [
  {
    department_name: "Play School",
    department_img: playschool_american,
    department_head: "playSchool",
  },
  {
    department_name: "Key Stage",
    department_img: keystage_american,
    department_head: "keyStage",
  },
  {
    department_name: "Elementary",
    department_img: elementary_american,
    department_head: "elementary",
  },
  {
    department_name: "Middle",
    department_img: middle_american,
    department_head: "middle",
  },
  {
    department_name: "High School",
    department_img: highschool_american,
    department_head: "highSchool",
  },
];
export const departmentsBritish = [
  {
    department_name: "Play School",
    department_img: playschool_american,
    department_head: "playSchool",
  },
  {
    department_name: "Foundation Stage",
    department_img: keystage_american,
    department_head: "foundation_stage",
  },
  {
    department_name: "Primary",
    department_img: elementary_american,
    department_head: "primary",
  },
  {
    department_name: "Lower Secondary",
    department_img: middle_american,
    department_head: "lower_secondary",
  },
  {
    department_name: "Upper Grades",
    department_img: highschool_american,
    department_head: "upper_grades",
  },
];

export const headAmerican = {
  playschool: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
  elementary: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
  keystage: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
  middle: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
  highschool: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
};
export const headBritish = {
  playschool: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
  foundation_stage: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
  primary: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
  lower_secondary: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
  upper_grades: {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mrs mona , head of playschool",
    img: american_head,
  },
};

//facts section

export const americanFacts = {
  yearIncrements: 20,
  studentIncrements: 5000,
  teacherIncrements: 200,
  graduateIncrements: 1500,
};
export const allFacts = {
  yearIncrements: 24,
  studentIncrements: 10000,
  teacherIncrements: 500,
  graduateIncrements: 3600,
};
export const britishFacts = {
  yearIncrements: 20,
  studentIncrements: 5000,
  teacherIncrements: 200,
  graduateIncrements: 1500,
};

//hero section

export const heroAmerican = {
  img: hero_american,
  header: "We Ensure better education for a better world",
  paragraph:
    "QIS provides students with the knowledge and skills essential for success, preparing them to excel in their futures and compete in a globalized world.",
};
export const heroHome = {
  img: hero,
  header: "We Ensure better education for a better world",
  paragraph:
    "QIS provides students with the knowledge and skills essential for success, preparing them to excel in their futures and compete in a globalized world.",
};
export const heroBritish = {
  img: hero_british,
  header: "We Ensure better education for a better world",
  paragraph:
    "QIS provides students with the knowledge and skills essential for success, preparing them to excel in their futures and compete in a globalized world.",
};

export const alumniData = [
  {
    img: user_1,
    name: "Emily Williams",
    paragraph:
      "  Elquds was where my dreams took flight. The teachers ignited my curiosity, encouraged my ambitions, and taught me to persevere.Their unwavering support and dedication built the foundation for my success at NASA. This school showed me that with determination and determination and a thirst for knowledge, no dream is too big. I am deeply grateful for the inspiration and values that shaped my journey to the stars.",
    position: "Lead Aerospace Engineer, NASA",
  },
  {
    img: user_2,
    name: "Emily Williams",
    paragraph:
      "  Elquds was where my dreams took flight. The teachers ignited my curiosity, encouraged my ambitions, and taught me to persevere.Their unwavering support and dedication built the foundation for my success at NASA. This school showed me that with determination and determination and a thirst for knowledge, no dream is too big. I am deeply grateful for the inspiration and values that shaped my journey to the stars.",
    position: "Lead Aerospace Engineer, NASA",
  },
  {
    img: user_3,
    name: "Emily Williams",
    paragraph:
      "  Elquds was where my dreams took flight. The teachers ignited my curiosity, encouraged my ambitions, and taught me to persevere.Their unwavering support and dedication built the foundation for my success at NASA. This school showed me that with determination and determination and a thirst for knowledge, no dream is too big. I am deeply grateful for the inspiration and values that shaped my journey to the stars.",
    position: "Lead Aerospace Engineer, NASA",
  },
  {
    img: user_4,
    name: "Emily Williams",
    paragraph:
      "  Elquds was where my dreams took flight. The teachers ignited my curiosity, encouraged my ambitions, and taught me to persevere.Their unwavering support and dedication built the foundation for my success at NASA. This school showed me that with determination and determination and a thirst for knowledge, no dream is too big. I am deeply grateful for the inspiration and values that shaped my journey to the stars.",
    position: "Lead Aerospace Engineer, NASA",
  },
  {
    img: user_4,
    name: "Emily Williams",
    paragraph:
      "  Elquds was where my dreams took flight. The teachers ignited my curiosity, encouraged my ambitions, and taught me to persevere.Their unwavering support and dedication built the foundation for my success at NASA. This school showed me that with determination and determination and a thirst for knowledge, no dream is too big. I am deeply grateful for the inspiration and values that shaped my journey to the stars.",
    position: "Lead Aerospace Engineer, NASA",
  },
  {
    img: user_4,
    name: "Emily Williams",
    paragraph:
      "  Elquds was where my dreams took flight. The teachers ignited my curiosity, encouraged my ambitions, and taught me to persevere.Their unwavering support and dedication built the foundation for my success at NASA. This school showed me that with determination and determination and a thirst for knowledge, no dream is too big. I am deeply grateful for the inspiration and values that shaped my journey to the stars.",
    position: "Lead Aerospace Engineer, NASA",
  },
  {
    img: user_4,
    name: "Emily Williams",
    paragraph:
      "  Elquds was where my dreams took flight. The teachers ignited my curiosity, encouraged my ambitions, and taught me to persevere.Their unwavering support and dedication built the foundation for my success at NASA. This school showed me that with determination and determination and a thirst for knowledge, no dream is too big. I am deeply grateful for the inspiration and values that shaped my journey to the stars.",
    position: "Lead Aerospace Engineer, NASA",
  },
  {
    img: user_4,
    name: "Emily Williams",
    paragraph:
      "  Elquds was where my dreams took flight. The teachers ignited my curiosity, encouraged my ambitions, and taught me to persevere.Their unwavering support and dedication built the foundation for my success at NASA. This school showed me that with determination and determination and a thirst for knowledge, no dream is too big. I am deeply grateful for the inspiration and values that shaped my journey to the stars.",
    position: "Lead Aerospace Engineer, NASA",
  },
  {
    img: user_4,
    name: "Emily Williams",
    paragraph:
      "  Elquds was where my dreams took flight. The teachers ignited my curiosity, encouraged my ambitions, and taught me to persevere.Their unwavering support and dedication built the foundation for my success at NASA. This school showed me that with determination and determination and a thirst for knowledge, no dream is too big. I am deeply grateful for the inspiration and values that shaped my journey to the stars.",
    position: "Lead Aerospace Engineer, NASA",
  },
];

export const testimonialsData = [
  {
    img: user_1,
    name: "Emily Williams",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
  {
    img: user_2,
    name: "Emily Williams",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
  {
    img: user_3,
    name: "Emily Williams",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
  {
    img: user_4,
    name: "Emily Williams",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
  {
    img: user_4,
    name: "Emily Williams",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
  {
    img: user_4,
    name: "Emily Williams",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
];
export const historyData = [
  {
    img: user_1,
    name: "2001",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
  {
    img: user_2,
    name: "2005",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
  {
    img: user_3,
    name: "2012",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
  {
    img: user_4,
    name: "2020",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
  {
    img: user_4,
    name: "2024",
    paragraph:
      "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
    division: "graduate, american",
  },
];

// navbar list

export const navbarListAmerican = [
  {
    name: "American",
    link: "hero",
    offset: 0,
  },
  {
    name: "Facts",
    link: "facts",
    offset: -260,
  },
  {
    name: "Departments",
    link: "departments",
    offset: -260,
  },
  {
    name: "Gallery",
    link: "albums",
    offset: -260,
  },
  {
    name: "Aluminis",
    link: "aluminis",
    offset: -260,
  },
];
export const navbarListBritish = [
  {
    name: "British",
    link: "hero",
    offset: 0,
  },
  {
    name: "Facts",
    link: "facts",
    offset: -260,
  },
  {
    name: "Departments",
    link: "departments",
    offset: -260,
  },
  {
    name: "Gallery",
    link: "albums",
    offset: -260,
  },
  {
    name: "Aluminis",
    link: "aluminis",
    offset: -260,
  },
];
export const navbarListHome = [
  {
    name: "Home",
    link: "hero",
    offset: 0,
  },
  {
    name: "About Us",
    link: "about",
    offset: -150,
  },
  {
    name: "Facts",
    link: "facts",
    offset: -260,
  },
  {
    name: "Divisons",
    link: "divisions",
    offset: -260,
  },
  {
    name: "Gallery",
    link: "albums",
    offset: -260,
  },
  {
    name: "Aluminis",
    link: "aluminis",
    offset: -260,
  },
  {
    name: "Contact Us",
    link: "contact",
    offset: -260,
  },
];
export const navbarListAbout = [
  {
    name: "About Us",
    link: "about",
    offset: 0,
  },
  {
    name: "Key Principles",
    link: "key-principles",
    offset: -150,
  },
  {
    name: "History",
    link: "history",
    offset: -260,
  },
  {
    name: "Values",
    link: "values",
    offset: -260,
  },

  {
    name: "Aluminis",
    link: "aluminis",
    offset: -260,
  },
  {
    name: "Contact Us",
    link: "contact",
    offset: -260,
  },
];

export const keyPrinciles = [
  {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_3:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mr mohamed , founder",
    img: school_head,
  },
  {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_3:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mr mohamed , founder",
    img: school_head,
  },
  {
    paragraph_1:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_2:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    paragraph_3:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Odio velit  saepe excepturi quos veniam eaque dicta quibusdam tempore beata saepe excepturi quos veniam eaque dicta quibusdam tempore beata ",
    signature: "Mr mohamed , founder",
    img: school_head,
  },
  // {
  //   img: user_4,
  //   name: "2020",
  //   paragraph:
  //     "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
  //   division: "graduate, american",
  // },
  // {
  //   img: user_4,
  //   name: "2024",
  //   paragraph:
  //     "   Choosing to pursue my degree at Elquds was one of the best decisions I've ever made. The supportive community, state-of-the-art facilities, and commitment to academic excellence have truly exceeded my expectations.",
  //   division: "graduate, american",
  // },
];

export const valueStatement = {
  mission: [
    {
      title: "",
      content:
        "To provide a safe and inclusive environment where students are empowered to reach their potential through high-quality education, fostering creativity, critical thinking, and respect.",
    },
  ],

  vision: [
    {
      title: "",

      content:
        "To provide a safe and inclusive environment where students are empowered to reach their potential through high-quality education, fostering creativity, critical thinking, and respect.",
    },
  ],

  coreValue: [
    {
      title: "Excellence:",
      content: " Achieving the highest standards.",
    },
    {
      title: "Integrity:",
      content: " Acting with honesty and fairness.",
    },
    {
      title: "Respect:",
      content: " Embracing diversity and inclusion.",
    },
    {
      title: "Collaboration:",
      content: " Working together in harmony.",
    },
    {
      title: "Innovation:",
      content: " Inspiring creativity and progress.",
    },
    {
      title: "Empathy:",
      content: " Showing care and understanding.",
    },
  ],
};

export const valueTitle = [
  {
    name: "Mission",
    img: mission,
  },
  {
    name: "Vision",
    img: vision,
  },
  {
    name: "Core value",
    img: core_value,
  },
];
